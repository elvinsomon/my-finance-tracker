...
const BudgetProgressBar = ({ budgeted, spent, currency, categoryName }) => {
  ...
  const getColorClass = () => {
    if (percentageUsed >= 100) return 'bg-expense-500 dark:bg-expense-400';
    if (percentageUsed >= 80) return 'bg-yellow-500 dark:bg-yellow-400';
    return 'bg-income-500 dark:bg-income-400';
  };

  const getTextColorClass = () => {
    if (percentageUsed >= 100) return 'text-expense-600 dark:text-expense-400';
  ...
  return (
    <div className="mb-4 group">
      ...
      <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4 mb-2 overflow-hidden">
        <div
          className={`h-4 rounded-full ${getColorClass()} transition-all duration-300 group-hover:scale-x-[1.01] origin-left`}
      ...
      {percentageUsed >= 100 && (
        <div className="mt-2 text-xs text-expense-600 dark:text-expense-400 font-semibold">
          Budget exceeded!
        </div>
      )}
